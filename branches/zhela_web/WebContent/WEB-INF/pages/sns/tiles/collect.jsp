...
<div id="wrap">
    ...
    <div id="content">
        ...
        <div>商品名称：<input type="text" id="g_title"/></div>
        <div>商品类别：<select><option>数码产品</option><option>流行服饰</option></select></div>
        <div>来源地址：<input type="text" value="<s:property value='source'/>"/></div>
        <div>图片：<input type="hidden" value="" id="g_i_s"/><img src="images/good.jpg" id="good_image"/><a id="g_i_p" href="#">&lt;</a><span id="g_i_c">2/3</span><a id="g_i_n" href="#">&gt;</a></div>
        <div>描述：<textarea>一件衣服，啊啊啊。如此而已。简短描述。可以为空。</textarea></div>
        <div>关注商品</div>
        <div>标记为：<select><option>衣服</option><option>猫</option></select><a href="#">新建标签</a></div>
        <div>跟踪价格：<input type="radio"  name="track_price"/>开<input type="radio"  name="track_price"/>关</div>
        <div>打分：@@@@@@ [使用星星打分]</div>
        <div>评论：<textarea></textarea></div>
        <div>分享给：<input type="radio"  name="share_good"/>我的好友<input type="radio"  name="share_good"/>关注我的人<input type="radio"  name="share_good"/>我关注的人<input type="radio"  name="share_good"/>不用了</div>
        <div><input type="submit" value="保存" /><input type="checkbox"  />私人收藏</div>
    </div>
